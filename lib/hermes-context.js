...
const GLOBAL_FILES = [
  {
    id: 'memory',
    name: 'MEMORY.md',
    description: 'Long-term memory Hermes persists across sessions',
    subdir: 'memories',
  },
  {
    id: 'user',
  ...
  },
  {
    id: 'soul',
    name: 'SOUL.md',
    description: 'Global personality and voice defaults',
    subdir: '',
  },
];
...
function resolveGlobalFilePath(hermesHome, spec) {
  const canonical = spec.subdir
    ? path.join(hermesHome, spec.subdir, spec.name)
    : path.join(hermesHome, spec.name);
  if (!spec.subdir) return canonical;

  const legacy = path.join(hermesHome, spec.name);
...
function listContextFiles(projectCwd) {
  ...
  for (const spec of GLOBAL_FILES) {
    const abs = resolveGlobalFilePath(hermesHome, spec);
