...
const { resolveAbsolutePath, isPathInside, browseDirectory, readWorkspaceFile } = require('./workspace-sandbox.js');
...
function getActiveAgentRoot() {
  const listing = listAgentWorkspaces();
  if (listing.terminalPath && fs.existsSync(listing.terminalPath)) {
    return path.resolve(listing.terminalPath);
  }
  const profileDir = getProfileWorkspaceDir();
  try {
  ...
  throw Object.assign(
    new Error('No Hermes working directory configured. Set terminal.cwd: hermes config set terminal.cwd ~/your-project'),
    { statusCode: 503 }
  );
}

function resolveInActiveRoot(requested) {
  const root = getActiveAgentRoot();
  const raw = String(requested || '.').replace(/\\/g, '/');
  if (path.isAbsolute(raw)) {
    const abs = resolveAbsolutePath(raw);
    if (!isPathInside(abs, root)) {
      throw Object.assign(new Error('Path is outside terminal.cwd'), { statusCode: 403 });
    }
    return abs;
  }
  const abs = path.resolve(root, raw);
  if (!isPathInside(abs, root)) {
    throw Object.assign(new Error('Path traversal blocked'), { statusCode: 403 });
  }
  return abs;
}

function listActiveAgentDirectory(relPath = '.') {
  const root = getActiveAgentRoot();
  const normalizedRel = relPath === '.' ? '.' : String(relPath).replace(/\\/g, '/');
  const target = resolveInActiveRoot(normalizedRel);
  ...
  const result = browseDirectory(target);
  const prefix = normalizedRel === '.' ? '' : normalizedRel;
  const entries = (result.entries || []).map((entry) => ({
    name: entry.name,
    path: prefix ? `${prefix}/${entry.name}` : entry.name,
    is_dir: entry.type === 'directory',
    size: entry.size,
    mtime_ns: entry.mtime != null ? entry.mtime * 1e6 : null,
  ...
  return {
    available: true,
    workspace: root,
    workspaceDisplay: displayRelativePath(root),
    path: normalizedRel,
    entries,
  };
}

function readActiveAgentFile(requestedPath) {
  const abs = resolveInActiveRoot(requestedPath);
  return readWorkspaceFile(abs);
}

function resolveActiveAgentFile(requestedPath) {
  return resolveInActiveRoot(requestedPath);
}
