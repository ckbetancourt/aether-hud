...
async function handleKanbanApi(req, res, pathname, u, helpers) {
  ...
  try {
    ...
    const reclaimMatch = pathname.match(/^\/api\/hermes\/kanban\/tasks\/([^/]+)\/reclaim$/);
    if (reclaimMatch && req.method === 'POST') {
      const taskId = decodeURIComponent(reclaimMatch[1]);
      const body = await readBody(req);
      sendJson(res, 200, await kanbanApi.reclaimTask(board, taskId, body || {}));
      return true;
    }

    const reassignMatch = pathname.match(/^\/api\/hermes\/kanban\/tasks\/([^/]+)\/reassign$/);
    if (reassignMatch && req.method === 'POST') {
      const taskId = decodeURIComponent(reassignMatch[1]);
      const body = await readBody(req);
      sendJson(res, 200, await kanbanApi.reassignTask(board, taskId, body || {}));
      return true;
    }
