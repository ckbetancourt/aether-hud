...
function writeWorkspaceFile(absPath, content) {
  if (!fs.existsSync(absPath)) {
    throw Object.assign(new Error('File does not exist'), { statusCode: 404 });
  }
  const st = fs.statSync(absPath);
  ...
  const ext = path.extname(absPath).toLowerCase();
  if (IMAGE_EXTENSIONS.has(ext)) {
    throw Object.assign(new Error('Image files cannot be edited here'), { statusCode: 400 });
  }

  const text = String(content ?? '');
  const buf = Buffer.from(text, 'utf8');
  if (buf.length > MAX_READ_BYTES) {
    throw Object.assign(new Error('File too large to save from HUD'), { statusCode: 413 });
  }
  if (!isLikelyTextFile(absPath, buf)) {
    throw Object.assign(new Error('File does not appear to be text'), { statusCode: 400 });
  }
